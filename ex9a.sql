...
-- limpeza dos dados para insert com transactions
delete from mecanico cascade;
delete from cliente cascade;
delete from setor cascade;
...
do $$
declare idade_temp int;
begin
	for idade_temp in select idade from cliente
	loop
		update cliente set idade = idade_temp + 1 where idade=idade_temp;
	end loop;
	for idade_temp in select idade from mecanico
	loop
		update mecanico set idade = idade_temp + 1 where idade=idade_temp;
	end loop;
end;
$$
language plpgsql;
...
--4)Insira um novo setor com cods 5 e atualize os mecânicos 
--alocados ao setor 2 para 5 em uma única transação.
begin transaction;
INSERT INTO setor VALUES (5, 'novo');
...
do $$
declare cidade_temp varchar;
begin
	for cidade_temp in select cidade from mecanico
	loop
		update mecanico set cidade = upper(cidade_temp) where cidade = cidade_temp;
	end loop;
	for cidade_temp in select cidade from cliente
	loop
		update cliente set cidade = upper(cidade_temp) where cidade = cidade_temp;
	end loop;
end;
$$
language plpgsql;
...
--6)Concatene na frente do nome dos mecânicos a abreviação 'Mec. ' 
--e no nome do clientes a abreviação 'Cli. ' em uma única transação.
begin transaction;

do $$
declare nome_temp varchar;
begin
	for nome_temp in select nome from mecanico
	loop
		update mecanico set nome = 'Mec.'||nome_temp where nome = nome_temp;
	end loop;
	for nome_temp in select nome from cliente
	loop
		update cliente set nome = 'Cli.'||nome_temp where nome = nome_temp;
	end loop;
end;
$$
language plpgsql;
